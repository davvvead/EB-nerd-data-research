...
### SUPPLY_12H: Retrospective observable supply window reduced from 24h to 12h
- **Authoritative Protocol Citations**:
...
- **Novelty Cache Policy**: Subset verification: May safely reuse cache/validation/user_article_novelty.parquet (24h Contrastive) ONLY after asserting that 100% of required 12h user-article pairs are present in the 24h cache.
- **Output Directory**: `outputs/robustness/supply_12h/`
- **Ambiguity & Boundary Analysis**: Unambiguous. Protocol explicitly defines 12h as a supply sensitivity window.

---

### SUPPLY_48H: Retrospective observable supply window expanded from 24h to 48h
- **Authoritative Protocol Citations**:
...
- **Frozen Components**:
  ...
  - Common support threshold (n >= 3 distinct users)
  - Headroom relevance retention constraint (>= 97%)
- **Novelty Cache Policy**: Expansion required: The 24h primary cache is missing 2,408,253 pairs (28.96% missing; 54.42% of impressions affected). A dedicated cache cache/validation/novelty_contrastive_48h.parquet must be computed and stored before running outcomes.
- **Output Directory**: `outputs/robustness/supply_48h/`
- **Ambiguity & Boundary Analysis**: Unambiguous. Protocol explicitly defines 48h as a supply sensitivity window.

---

### BERT: Semantic novelty evaluated using Multilingual BERT embeddings instead of Contrastive embeddings
- **Authoritative Protocol Citations**:
...
- **Novelty Cache Policy**: Reuses novelty_k3 column from cache/validation/user_article_novelty.parquet (which already pre-computed k=3, 5, 10 for the 24h universe).
- **Output Directory**: `outputs/robustness/novelty_k3/`
- **Ambiguity & Boundary Analysis**: Unambiguous. Protocol and implementation decisions explicitly pre-computed k=3, 5, 10 in the primary novelty cache.

---

### NOVELTY_K10: Semantic novelty history sensitivity: k=10 instead of primary k=5
- **Authoritative Protocol Citations**:
  - `analysis_protocol_v1.yaml: lines 24-27 (history_similarity_k: 5, history_similarity_k_sensitivity: [3, 10])`
  - `IMPLEMENTATION_DECISIONS.md: Item 4 (Derive and cache novelty for k=5 as well as k=3 and k=10)`
- **Changed Components**:
...
- **Frozen Components**:
  ...
  - Observed slates, choice consumption sets, and matched pairs
  - Headroom constraints and common support
- **Novelty Cache Policy**: Reuses novelty_k10 column from cache/validation/user_article_novelty.parquet.
- **Output Directory**: `outputs/robustness/novelty_k10/`
- **Ambiguity & Boundary Analysis**: Unambiguous. Protocol and implementation decisions explicitly pre-computed k=3, 5, 10 in the primary novelty cache.

---

### COMMON_SUPPORT_5: Common-support sensitivity threshold for Headroom: candidate article must appear in-view for >= 5 distinct users in 30-min window (vs >= 3)
- **Authoritative Protocol Citations**:
...
- **Novelty Cache Policy**: Reuses cache/validation/user_article_novelty.parquet (the common support filter strictly narrows the candidate pool).
- **Output Directory**: `outputs/robustness/common_support_5/`
- **Ambiguity & Boundary Analysis**: Unambiguous. Protocol explicitly defines sensitivity_n: 5 under common_support.

---

### HEADROOM_RETENTION_99: Headroom modeled relevance retention constraint tightened from 97% to 99% (tolerance 0.01)
- **Authoritative Protocol Citations**:
...
- **Novelty Cache Policy**: Reuses cache/validation/user_article_novelty.parquet.
- **Output Directory**: `outputs/robustness/headroom_retention_99/`
- **Ambiguity & Boundary Analysis**: Unambiguous. Protocol explicitly lists 0.01 under sensitivity_relevance_tolerances.

---

### HEADROOM_RETENTION_95: Headroom modeled relevance retention constraint relaxed from 97% to 95% (tolerance 0.05)
- **Authoritative Protocol Citations**:
  - `analysis_protocol_v1.yaml: lines 109-112 (primary_relevance_tolerance: 0.03, sensitivity_relevance_tolerances: [0.01, 0.05])`
  - `LOCKED_ANALYSIS_SPEC.md: line 99 (Robustness tolerances: 1% and 5%)`
- **Changed Components**:
...
- **Frozen Components**:
  ...
  - Maximum swap budget (2 swaps)
- **Novelty Cache Policy**: Reuses cache/validation/user_article_novelty.parquet.
- **Output Directory**: `outputs/robustness/headroom_retention_95/`
- **Ambiguity & Boundary Analysis**: Unambiguous. Protocol explicitly lists 0.05 under sensitivity_relevance_tolerances.
